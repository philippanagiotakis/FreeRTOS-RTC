...
void vTttOsTask( void * argument )
{
   ...
   //
   // char msg[64];
   // sprintf(msg, "Tick timing task started!\r\n");
   ...
   // sprintf(msg, "After Config! %ld , %ld\r\n",config.subsecondsPerSecond, config.subsecondsPerSecond / configTICK_RATE_HZ);
   // HAL_UART_Transmit(&huart3, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
   ...
   // sprintf(msg, "After ConfigAssert!\r\n");
   // HAL_UART_Transmit(&huart3, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
